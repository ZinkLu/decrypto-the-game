...
export default function TeammateWaiting() {
  ...
  // 模拟波形状态变化（实际项目中应该从游戏状态同步）
  useEffect(() => {
    // 模拟第一个完成
    const timer1 = setTimeout(() => {
      setWaveforms((prev) =>
        prev.map((w) =>
          w.id === 1 ? { ...w, state: 'completed' as OscilloscopeState, statusText: '已完成' } : w
        )
      );
      setCompletedCount(1);
    }, 5000);
    ...
    const timer2 = setTimeout(() => {
      setWaveforms((prev) =>
        prev.map((w) =>
          w.id === 2 ? { ...w, state: 'completed' as OscilloscopeState, statusText: '已完成' } : w
        )
      );
      setCompletedCount(2);
    }, 10000);
    ...
    const timer3 = setTimeout(() => {
      setWaveforms((prev) =>
        prev.map((w) =>
          w.id === 3 ? { ...w, state: 'completed' as OscilloscopeState, statusText: '已完成' } : w
        )
      );
      setCompletedCount(3);
    }, 15000);
